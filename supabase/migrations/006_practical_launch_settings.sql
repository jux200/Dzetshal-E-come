...
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'theme_settings_pkey'
      and conrelid = 'public.theme_settings'::regclass
  ) then
    alter table theme_settings add constraint theme_settings_pkey primary key (id);
  end if;
end $$;
