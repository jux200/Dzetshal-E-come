...
async function applyAdminBrandTheme(){
  try{ const {data}=await sb.from('theme_settings').select('logo_theme,logo_size,show_store_name').eq('id','main').maybeSingle(); const img=document.getElementById('adminBrandWordmark'); if(img){ img.src=(data?.logo_theme==='peach'?'../img/brand-logo-peach.png':'../img/brand-logo-green.png'); img.style.width=Math.max(105,Math.min(220,Number(data?.logo_size||165)))+'px'; img.style.display=data?.show_store_name===false?'none':'block'; } }catch(e){}
}
function bindShell(page){
  setActive(page); initTheme(); applyAdminBrandTheme();
